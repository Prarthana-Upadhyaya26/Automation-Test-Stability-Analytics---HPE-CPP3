...
ML_MODULES = [
    ...
    {"id": "ml5", "icon": "🚀", "title": "ML3 · Test Prioritization", "help": "Run prioritization engine"},
    {"id": "ml3", "icon": "💬", "title": "ML4 · Failure Clustering", "help": "Run failure message clustering"},
    {"id": "ml4", "icon": "🏥", "title": "ML5 · Run Health", "help": "Run anomaly detection"},
]
...
def _render_priority_card() -> str:
    ...
    <div class='spec-card'>
      <div class='spec-card-header'>
        <div>
          <div class='spec-title-row'>
            <span class='spec-badge spec-badge-blue'>ML3</span>
...
def _render_module_implementation(module_id: str) -> None:
    if module_id == "ml1":
        ...
        _implementation_expander(
            ...
            html=_render_impl_card(
                ...
                formula="preprocess: lower → em-dash→space → strip punct → digits→NUM → drop 1-char<br><br>confidence = 1 − min(centroid_dist / percentile_95, 1.0)<br>low-conf threshold: 0.30<br><br>artefacts: tfidf_vectorizer.pkl · kmeans_model.pkl · cluster_labels.pkl · train_distances.pkl",
                metrics=[
...
def render_failure_clustering(failure_clusters: pd.DataFrame, df_results: pd.DataFrame) -> None:
    failure_clusters = failure_clusters.copy()
    failure_clusters["cluster_category"] = failure_clusters["cluster_category"].replace({"unknown": "assertion"})
...
def render_run_health_intelligence(df_runs: pd.DataFrame, anomalies: pd.DataFrame) -> None:
    ...
    z_only = anomaly_rows[anomaly_rows["detection_method"] == "Z-score"]
    if_only = anomaly_rows[anomaly_rows["detection_method"] == "Isolation Forest"]
    both = anomaly_rows[anomaly_rows["detection_method"] == "Both"]

    st.markdown("**Run health anomaly summary**")
    st.markdown(
        f"- Z-score only: **{len(z_only)}** runs  \
"
        f"- Isolation Forest only: **{len(if_only)}** runs  \
"
        f"- Both methods: **{len(both)}** runs"
    )

    display = anomaly_rows.copy()
    display["pass_rate_pct"] = display["pass_rate_pct"].round(1)
    display["z_score"] = display["z_score"].round(2)
    display["zscore_anomaly"] = display["zscore_anomaly"].map({True: "Yes", False: "No"})
    display["if_anomaly"] = display["if_anomaly"].map({True: "Yes", False: "No"})
    display["anomaly_score"] = display["anomaly_score"].round(2)

    display = display[ 
        ["run_number", "run_id", "build_no", "pass_rate_pct", "z_score", "zscore_anomaly", "if_anomaly", "detection_method", "anomaly_score", "failed_tests"]
    ].rename(
        columns={
            "run_number": "Run #",
            "run_id": "Run ID",
            ...
            "z_score": "Z-score",
            "zscore_anomaly": "Z-score flag",
            "if_anomaly": "Isolation Forest",
            "detection_method": "Detected by",
...
def _mark_module_ran(module_id: str) -> None:
    st.session_state[f"ran_{module_id}"] = True


def _render_module_slot(module_id: str, df_runs: pd.DataFrame, df_results: pd.DataFrame, top_n: int) -> None:
    module_meta = next((m for m in ML_MODULES if m["id"] == module_id), None)
    if module_meta is not None:
        st.markdown(f'<div class="module-header">{module_meta["title"]}</div>', unsafe_allow_html=True)
    _render_module_implementation(module_id)
    ran_key = f"ran_{module_id}"
    ran = st.session_state.get(ran_key, False)
    if not ran:
        st.button(
            "Run this analysis",
            key=f"slot_run_{module_id}",
            use_container_width=True,
            on_click=_mark_module_ran,
            args=(module_id,),
        )
        st.markdown(
            '<div class="ml-slot"><div class="ml-slot-empty">Click the module button above or the Run button below to execute this analysis.</div></div>',
            unsafe_allow_html=True,
        )
        return
...
def main() -> None:
    ...
    for module in ML_MODULES:
        st.session_state.setdefault(f"ran_{module['id']}", False)

    st.sidebar.markdown("---")
    top_n = 10
    st.sidebar.markdown(f"**Top N results:** {top_n}")
