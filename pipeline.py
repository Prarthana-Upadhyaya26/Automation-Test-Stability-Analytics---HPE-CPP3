...
def parse_jira_defect(raw: dict) -> Optional[dict]:
    ...
    # reporter_email — try both flat and nested field variants
    reporter_raw = (
        raw.get("reporter_name")
        or raw.get("reporter_email")
    ...
    reporter_email = _normalise_email(reporter_raw)
...
def map_defects_to_test_results(
    ...
    *,
    ...
    window_days:     int           = DEFECT_WINDOW_DAYS,
    ...
    candidates_sql = """
        ...
        WHERE  tr.status = 'FAIL'
          AND  ABS(JULIANDAY(d.created) - JULIANDAY(r.timestamp)) <= :window_days
    ...
    cand_cursor = conn.execute(candidates_sql, {"window_days": window_days})
    ...
    for row in cand_cursor:
        ...
        date_within_window = 1  # guaranteed by SQL WHERE clause
        ...
        confirmed  = 1 if (email_match and date_within_window and score >= 0.5) else 0
