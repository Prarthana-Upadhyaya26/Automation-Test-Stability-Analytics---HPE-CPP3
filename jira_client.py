...
def _build_jql(
    ...
    """
    Build a JQL string that retrieves only automation-related Bug issues.

    Base filter
    ───────────
    project IN (...) AND issuetype = Bug AND created >= -Nd
    AND labels in ("automation")

    Optional additions
    ──────────────────
    reporter_email  → AND reporter = "email@domain.com"
    extra_jql       → raw JQL fragment ANDed at the end (for advanced use)
    """
    proj_list = ", ".join(f'"{p}"' for p in projects)
    clauses = [
        f"project IN ({proj_list})",
        "issuetype = Bug",
        f"created >= -{since_days}d",
        'labels in ("automation")',
    ]
    if reporter_email:
        clauses.append(f'reporter = "{reporter_email}"')
    if extra_jql:
...
def write_back_confirmed_mappings(
    ...
    *,
    ...
    # Load confirmed mappings that haven't been written back yet
    rows = conn.execute(
        """
